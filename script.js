...
function revealWords() {
  ...
  words.forEach(w => {
    ...
    if (rect.top < winH * 0.8) w.classList.add('visible');
  });
}
window.addEventListener('scroll', revealWords);
revealWords();

// Certifications carousel (placeholder)
const certifs = [
...
let certIdx = 0;
function changeCertif(dir) {
  certIdx = (certIdx + dir + certifs.length) % certifs.length;
  const c = certifs[certIdx];
  document.getElementById('certif-icon').textContent = c.icon;
  document.getElementById('certif-title').textContent = c.title;
  document.getElementById('certif-desc').textContent = c.desc;
  document.getElementById('certif-circle').className = 'certif-circle' + (c.empty ? ' empty' : '');
  document.getElementById('certif-info').className = 'certif-info' + (c.empty ? ' empty' : '');
}
